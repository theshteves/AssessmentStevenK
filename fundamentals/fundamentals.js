...
function minMax(values) {
  // Assumption: Since not all values in the array are known to be numbers & I don't have the time to be comprehensive, I took extra precaution in the face of ambiguous constraints to only accept numeric types (to avoid booleans implicitly casting to numbers & save me the time of enumerating tests for unpacking nested containers, sanitizing malicious strings, etc. ...although a simple RegEx might do).  This filter prevents compliant strings from getting implicitly cast, as well as booleans (e.g. true -> 1). Floats (e.g. 3.14159), exponentials (e.g. 2.048e3), negatives, & number-resolving expressions (e.g. Math.E) are supported. The only good-faith assumption I make is that I'll be passed a valid JavaScript Array.
  numbers = values.filter(value => typeof(value) === 'number') // no semi-colons necessary, but you'll see me prefer them in all assessments

  return [Math.min(...numbers), Math.max(...numbers)]
  // While this optimizes for readability & developer time cost, I still managed to needlessly sacrifice time efficiency & nobody likes "silent failures" (I could at least detect them & leave a console.warn). I have a frontend & backend to do on top of other interviews so no testing suite today. I prefer writing technical docs to just dumping this dense just-in-time prose between lines of code here, but I'm not getting paid to do this assessment so it'll do
}

// one personal test: minMax([2,false,"12",Math.PI,1]) ➞ [1, 3.1415926...]

// Just to flex:
minMax = array => (nums => [Math.min(...nums), Math.max(...nums)])(array.filter(x => typeof(x) === 'number'));
...
const sortObjects = (values, sortBy) => values.sort((a, b) => a[sortBy] > b[sortBy]);

// When I'm on my best behavior for readability:
/*
const sortObjects = (values, sortBy) => {
  values.sort((a, b) => a[sortBy] > b[sortBy]); // sort objects by "sortBy" field name (ascending order)
  return values;
}
*/
